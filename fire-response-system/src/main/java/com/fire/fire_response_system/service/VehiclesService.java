...
public class VehiclesService {
    ...
    public VehicleResponse create(VehicleCreateRequest req) {
        if (!stationRepository.existsById(req.getStationId())) {
            throw new IllegalArgumentException("존재하지 않는 stationId");
        }
    ...
    public List<VehicleListItem> list(Long stationId, Integer status, String typeName, String callSignLike) {
        ...
        return em.createQuery(cq).getResultList().stream()
                .map(v -> new VehicleListItem(v.getId(), v.getStationId(), v.getTypeName(), v.getCallSign(), v.getStatus(), v.getRallyPoint()))
                .toList();
    }
    ...
    public VehicleResponse update(Long id, VehicleUpdateRequest req) {
        ...
        // callSign 변경 시 중복 검사(stationId 내)
        ...
        return toResponse(v); // JPA dirty checking
    }
    ...
    private static VehicleResponse toResponse(Vehicle v) {
        return new VehicleResponse(
                v.getId(), v.getStationId(), v.getTypeName(), v.getCallSign(),
